...
OFFSET ?"""
    ...
    @Test
    fun testParseSimpleQueryWithPermission() {
        val parser = QueryParser()

        try {
            parser.parse(simpleQuery, QueryPermissions().entity(UserModel()) {
                it.all().exclude { it::lastLoginTime }
            })
            ...
            assertTrue(e is NoPermissionException)
            assertTrue((e as NoPermissionException).message!!.contains(" lastLogin "))
            assertTrue(e.message!!.contains(UserModel::class.java.name))
        }
    }

    @Test
    fun testParseSimpleQueryWithPermissionConditions() {
        val parser = QueryParser()

        val queries = parser.parse(simpleQuery, QueryPermissions().entity(UserModel()) {
            it.all().include({ it::id }) {
                it.rule(JSON.parseObject("""{"id": {"_gt": 2}}"""))
            }
        })
